...
Au démarrage, une base neuve reçoit la migration 1, puis le service ponctuel `migrate` applique les migrations 2 à 8. Sur un volume existant, il applique seulement les migrations manquantes. Les tables séparent les sources physiques, extractions versionnées, segments avec page ou ligne, observations et provenance des observations. Une source n’est pas encore une facture métier.
...
Le XLSX reste une source physique distincte. Ses lignes ne remplacent pas les observations des PDF/JPG et ne sont pas encore fusionnées avec elles. Le rapprochement des représentations et le signalement des conflits sont traités dans l'étape suivante.
...
La dernière commande exécute aussi les tests SQL, du contrat et du serveur dont elle dépend. Elle vérifie un lot synthétique de 50 documents, un PDF texte, un PDF scanné français, des JPG français, anglais, ambigu et illisible, un XLSX d'achats, une erreur technique OCR après trois tentatives, ainsi que la reprise idempotente. Le scénario arabe est conservé mais reporté. Le test final charge un JPG français, un XLSX et un relevé CSV par le Nginx du service web, puis vérifie l’API, le worker, les valeurs brutes et normalisées, les numéros de ligne, la classification bancaire et leur stockage PostgreSQL. Ces tests utilisent leurs propres services et volume. Pour les retirer :

```sh
docker compose -f compose.test.yaml down -v
```

L’import des relevés CSV prépare EX-03 sans créer encore d’allocation. La liaison XLSX/PDF, le rapprochement bancaire, les contrôles comptables et fiscaux, l’agent, l’Explainer et la revue humaine seront ajoutés dans les étapes fonctionnelles suivantes.
