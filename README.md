...
Le projet est en construction. On peut créer un lot, déposer un PDF texte, suivre son traitement par le worker et consulter un aperçu du texte extrait. Les références de page sont conservées en base. Les scans sans couche texte reçoivent un état `NON_TRAITE` et un motif ; l'OCR, les autres formats et les analyses métier ne sont pas encore disponibles. Aucune exigence métier EX-01 à EX-08 n'est déclarée validée.
...
La migration 3 conserve les passages extraits avec leur page ou ligne. La confiance, si elle est réellement fournie par l'outil, est exprimée en pourcentage ; sinon elle reste `NULL`. La migration 4 ajoute le nom facultatif des lots existants ; les nouveaux lots créés par l'API exigent un nom.

## PDF texte

Dans un lot ouvert, l'interface accepte un PDF de 15 Mo maximum. L'API vérifie sa signature et le dédoublonne par empreinte dans le lot ; le worker lit réellement le texte avec Poppler, page par page, puis conserve le résultat et les références de page dans PostgreSQL. Le texte affiché est un aperçu de 2 000 caractères. Un PDF de plus de 30 pages, sans texte ou avec une page sans texte reste `NON_TRAITE` avec un motif explicite. Cette étape ne reconnaît pas encore les images et ne déduit aucun montant comptable.
...
Cette seconde commande exécute également les tests SQL et les tests du contrat dont elle dépend. Elle vérifie l'upload, le traitement réel d'un PDF texte, le refus d'un doublon et l'état d'un PDF sans texte dans des conteneurs isolés. Les tests facultatifs ne touchent pas aux volumes de l'application. Le code de sortie de chaque commande est celui de ses tests. Pour retirer ensuite les conteneurs de test : `docker compose -f compose.test.yaml down`.
